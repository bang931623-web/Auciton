...
def onbid_props(it: ob.OnbidItem) -> dict:
    p = {
        ...
        "입찰마감일": no.date(it.bid_end),
        "용도": no.txt(it.category),
        "법원/기관": no.txt(it.org),
        "상태": no.sel("진행"),
        "물건키": no.txt(it.key),
        "추적키": no.txt(""),
...
def main() -> int:
    ...
    res_db = n.find_db(no.UNIFIED_TITLE)
    if not res_db:
        print(f"!! 통합 결과 DB '{no.UNIFIED_TITLE}' 를 찾을 수 없습니다. "
              "0단계 워크플로를 먼저 실행하세요.")
        return 1
    print(f"결과 DB: {no.UNIFIED_TITLE}")
